...
namespace testSDK
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            request.AccessToken = "23.7fa667ebfa3c334dda4b6a352c56886e.1363265.7668257667.163557-11571726"; // 文档中对应字段：access_token，实际使用时请替换成真实参数
            request.Offset = 0; // 文档中对应字段：offset，实际使用时请替换成真实参数
            request.Count = 3; // 文档中对应字段：count，实际使用时请替换成真实参数

            

            Task task = TestGetTemplateListRequest(request);
            task.Wait();
        }

        public static async Task TestGetTemplateListRequest(GetTemplateListRequest request)
        {
            try {
