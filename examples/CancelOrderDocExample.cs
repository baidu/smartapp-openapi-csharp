...
namespace testSDK
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            request.AccessToken = "21.056c3424c51c61232d5efd2f8358ebd3.2737423.0146887331.301318-38532440"; // 文档中对应字段：access_token，实际使用时请替换成真实参数
            request.OrderID = 1278182204736; // 文档中对应字段：orderId，实际使用时请替换成真实参数
            request.PmAppKey = "MMUpGO"; // 文档中对应字段：pmAppKey，实际使用时请替换成真实参数

            

            Task task = TestCancelOrderRequest(request);
            task.Wait();
        }

        public static async Task TestCancelOrderRequest(CancelOrderRequest request)
        {
            try {
