...
namespace testSDK
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            request.AccessToken = "23.5fa083ebfa7c517dda2b7a006c41366e.6328745.2418480144.087782-26223227"; // 文档中对应字段：access_token，实际使用时请替换成真实参数
            request.Offset = 0; // 文档中对应字段：offset，实际使用时请替换成真实参数
            request.Count = 3; // 文档中对应字段：count，实际使用时请替换成真实参数

            

            Task task = TestGetTemplateLibraryListRequest(request);
            task.Wait();
        }

        public static async Task TestGetTemplateLibraryListRequest(GetTemplateLibraryListRequest request)
        {
            try {
