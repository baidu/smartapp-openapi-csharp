...
namespace testSDK
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            request.AccessToken = "22.140f46224a5525a2c3ad7c824371eb27.8754714.6873233457.707547-86634300"; // 文档中对应字段：access_token，实际使用时请替换成真实参数

            
            
            // PostRequests
            CheckTextPostData postData = new CheckTextPostData {
                Content = "待检测文本", // 文档中对应字段：content，实际使用时请替换成真实参数
                Type = new List<string> { "risk", "lead", }, // 文档中对应字段：type，实际使用时请替换成真实参数
            };

            string postDataStr = JsonConvert.SerializeObject(postData);
            request.jsonBody = postDataStr;

            Task task = TestCheckTextRequest(request);
            task.Wait();
        }

        public static async Task TestCheckTextRequest(CheckTextRequest request)
        {
            try {
