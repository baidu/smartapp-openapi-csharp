...
namespace testSDK
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            request.AccessToken = "21.3fa661ebfa2c458dda8b4a774c11012e.0276821.7384510006.355305-65271450"; // 文档中对应字段：access_token，实际使用时请替换成真实参数
            // Post Form
            request.AppID = 12001348; // 文档中对应字段：app_id，实际使用时请替换成真实参数
            request.Path = "/pages/detail/detail?id=251266"; // 文档中对应字段：path，实际使用时请替换成真实参数

            

            Task task = TestDeleteResourceRequest(request);
            task.Wait();
        }

        public static async Task TestDeleteResourceRequest(DeleteResourceRequest request)
        {
            try {
