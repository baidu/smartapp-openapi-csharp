...
namespace testSDK
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            request.AccessToken = "21.1fa886ebfa3c534dda4b7a741c63438e.7132601.1237821783.485570-24407182"; // 文档中对应字段：access_token，实际使用时请替换成真实参数
            // Post Form
            request.AppID = 16365268; // 文档中对应字段：app_id，实际使用时请替换成真实参数
            request.ID = 16854; // 文档中对应字段：id，实际使用时请替换成真实参数
            request.Path = "/pages/index/index"; // 文档中对应字段：path，实际使用时请替换成真实参数

            

            Task task = TestDeleteMaterialRequest(request);
            task.Wait();
        }

        public static async Task TestDeleteMaterialRequest(DeleteMaterialRequest request)
        {
            try {
